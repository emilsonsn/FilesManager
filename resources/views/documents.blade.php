...
@section('content')
            ...
            <table class="table table-striped">
                ...
                <tbody>
                    @foreach ($documents as $doc)
                        <tr
                            class="{{ strpos($doc->situationAC . ' ' . $doc->situationAI, 'Descartado') !== false ? 'tr-grey' : '' }}">
                            ...
                            <td class="text-center"
                                style="font-weight: 600; color: {{ !$doc->situationAC ? '' : ($doc->situationAC == 'Ativo' ? 'green' : 'red') }} !important;">
                                {{ $doc->situationAC }}</td>
                            <td class="text-center"
                                style="font-weight: 600; color: {{ !$doc->situationAI ? '' : ($doc->situationAI == 'Ativo' ? 'green' : 'red') }} !important;">
                                {{ $doc->situationAI }}</td>
                            @php
                                $documentCollection = DocumentCollection::whereNull('return_date')
                                    ->whereHas('documentLoans', function ($query) use ($doc) {
                            ...
                            <td class="text-center fs-4">
                                @if ($auth->read_doc)
                                    <a href="#" class="edit-document" style="color: rgb(50, 127, 243) !important;"
                                        ...
                                        data-initial_date="{{ $doc->initial_date }}" data-id="{{ $doc->id }}"
                                        data-observations="{{ $doc->observations }}"
                                        data-project_id="{{ $doc->project_id }}"
                                        data-temporality_id="{{ $doc->temporality_id }}"
                                        data-doc_number="{{ $doc->doc_number }}"
                                        data-holder_name="{{ $doc->holder_name }}"
                                        data-description="{{ $doc->description }}" data-box="{{ $doc->box }}"
                            ...
                            <td class="text-center">
                                @if ($auth->edit_doc)
                                    <a href="#" class="me-2 edit-document" data-tags="{{ $doc->tags }}"
                                        data-archive_date="{{ $doc->archive_date }}"
                                        data-initial_date="{{ $doc->initial_date }}" data-id="{{ $doc->id }}"
                                        data-observations="{{ $doc->observations }}"
                                        data-project_id="{{ $doc->project_id }}"
                ...
                <form action="{{ route('create.document') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <ul id="fileListEdit" class="list-group mt-2">
    ...
    <div class="modal fade" id="generateBoxPrintModal" tabindex="-1" aria-labelledby="generateBoxPrintModalLabel"
        aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <form id="boxPrintForm">
                        <div class="row">
                            <div class="mb-3 col-md-4">
                                ...
                                <input type="text" class="form-control" id="central_archive" name="central_archive"
                                    required>
                            </div>
                            <div class="mb-3 col-md-4">
                            ...
                            <div class="mb-3 col-md-4">
                                ...
                                <input type="text" class="form-control" id="classification_code"
                                    name="classification_code" required>
                            </div>
                            <div class="mb-3 col-md-4">
                                <label for="area" class="form-label">Área</label>
                            ...
                            <div class="mb-3 col-md-4">
                                ...
                                <input type="text" class="form-control" id="custody_period" name="custody_period"
                                    required>
                            </div>
                            <div class="mb-3 col-md-4">
                                <label for="archive_year" class="form-label">Ano de Arquivamento</label>
                                <input type="text" class="form-control" id="archive_year" name="archive_year"
                                    required>
                            </div>
                            <div class="mb-3 col-md-4">
    ...
    <div class="modal fade" id="generateCabinetPrintModal" tabindex="-1"
        aria-labelledby="generateCabinetPrintModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <form id="cabinetPrintForm">
                        <div class="row">
                            <div class="mb-3 col-md-4">
                                ...
                                <input type="text" class="form-control" id="central_archive_cabinet"
                                    name="central_archive_cabinet" required>
                            </div>
                            <div class="mb-3 col-md-4">
                                <label for="organization_cabinet" class="form-label">Opcional</label>
                                <input type="text" class="form-control" id="organization_cabinet"
                                    name="organization_cabinet">
                            </div>
                            <div class="mb-3 col-md-4">
                                <label for="classification_code_cabinet" class="form-label">Código de
                                    Classificação</label>
                                <input type="text" class="form-control" id="classification_code_cabinet"
                                    name="classification_code_cabinet" required>
                            </div>
                            <div class="mb-3 col-md-4">
                                <label for="area_cabinet" class="form-label">Área</label>
                                <select type="text" class="form-control" id="area_cabinet" name="area_cabinet"
                                    required>
                                    <option value="MEIO">Meio</option>
                                    <option value="FIM">Fim</option>
                                </select>
                            ...
                            <div class="mb-3 col-md-4">
                                ...
                                <input type="text" class="form-control" id="custody_period_cabinet"
                                    name="custody_period_cabinet" required>
                            </div>
                            <div class="mb-3 col-md-4">
                                <label for="archive_year_cabinet" class="form-label">Ano de Arquivamento</label>
                                <input type="text" class="form-control" id="archive_year_cabinet"
                                    name="archive_year_cabinet" required>
                            </div>
                            <div class="mb-3 col-md-4">
                                <label for="location_cabinet" class="form-label">Localização</label>
                                <input type="text" class="form-control" id="location_cabinet" name="location_cabinet"
                                    required>
                            </div>
                            <div class="mb-3 col-md-4">
                                <label for="final_destination_cabinet" class="form-label">Destinação Final</label>
                                <select class="form-control" id="final_destination_cabinet"
                                    name="final_destination_cabinet" required>
                                    <option value="Permanente">Permanente</option>
                                    <option value="Eliminação">Eliminação</option>
                                </select>
                            </div>
                            ...
                            <div class="mb-3 col-md-4">
                                ...
                                <input type="text" class="form-control" id="drawer_number" name="drawer_number"
                                    required>
                            </div>
                            <div class="mb-3 col-md-8">
                                <label for="observations_cabinet" class="form-label">Observações</label>
                                <input type="text" class="form-control" id="observations_cabinet"
                                    name="observations_cabinet">
                            </div>
                        </div>
...
<script>
    ...
    document.getElementById('printBox').addEventListener('click', function() {
        console.log(caixas)
        var url = '{{ route("box") }}' + '?data=' + encodeURIComponent(JSON.stringify(caixas));
        var width = 1000;
        var height = 600;
        var left = (screen.width - width) / 2;
        var top = (screen.height - height) / 2;
    ...
    document.getElementById('printCabinet').addEventListener('click', function() {
        var url = '{{ route("cabinet") }}' + '?data=' + encodeURIComponent(JSON.stringify(armarios));
        var width = 1000;
        var height = 600;
        var left = (screen.width - width) / 2;
        var top = (screen.height - height) / 2;
                ...
                .then(data => {
                    ...
                    if (data.length > 0) {
                        data.forEach(file => {
                            ...
                            listItem.innerHTML = `${new Date(file.created_at).toLocaleDateString('pt-BR')} - ${file.name} <a href="/storage/${file.file_path}" target="_blank" class="btn btn-sm btn-primary">Abrir</a>`;
    ...
    editButtons.forEach(function(button) {
        button.addEventListener('click', function() {
            ...
            fetch(`/documents/${id}/files`)
                .then(response => response.json())
                .then(data => {
                    var fileList = document.getElementById('fileListEdit');
                    fileList.innerHTML = '';
                    if (data.length > 0) {
                        data.forEach(file => {
                            var listItem = document.createElement('li');
                            listItem.className = 'list-group-item d-flex justify-content-between align-items-center';
                            listItem.innerHTML = `<div>${file.name}</div> <a href="${file.file_path}" target="_blank" class="btn btn-sm btn-primary">Ver</a>`;
                            fileList.appendChild(listItem);
                        });
                    } else {
                        fileList.innerHTML = '<li class="list-group-item">Nenhum arquivo encontrado</li>';
                ...
                .then(data => {
                    ...
                    if (data.length > 0) {
                        data.forEach(file => {
                            var listItem = document.createElement('li');
                            listItem.className = 'list-group-item d-flex justify-content-between align-items-center';
                            listItem.innerHTML = `${new Date(file.created_at).toLocaleDateString('pt-BR')} - ${file.name} <div> <a href="${file.file_path}" target="_blank" class="btn btn-sm btn-primary">Abrir</a> <button class="btn btn-sm btn-danger delete-file" data-file-id="${file.id}">Apagar</button> </div>`;
                            fileList.appendChild(listItem);
                        });
                    } else {
                        fileList.innerHTML = '<li class="list-group-item">Nenhum arquivo encontrado</li>';
